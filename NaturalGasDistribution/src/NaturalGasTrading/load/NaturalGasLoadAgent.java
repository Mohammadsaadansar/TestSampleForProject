...
import NaturalGasTrading.ontology.*;
import NaturalGasTrading.supplier.NaturalGasSupplierAgent;

public class NaturalGasLoadAgent extends Agent {
// The list of known seller agents
private Vector sellerAgents = new Vector();
boolean f=true;
// The GUI to interact with the user
private NaturalGasLoadGui myGui;

/** 
 * The following parts, where the SLCodec and BookTradingOntology are registered
**/
private Codec codec = new SLCodec();
private Ontology ontology = NaturalGasTradingOntology.getInstance();
...
public void action() {
NaturalGasInfo info = (NaturalGasInfo) myAgent.getO2AObject();
if (info != null) {
...
System.out.println("NaturalGas Buyer-agent "+getAID().getName()+" is ready.");
...
if(f)
{
myGui = new NaturalGasLoadGuiImpl();
myGui.setAgent(this);
...
System.out.println("NaturalGastyricity Buyer-agent "+getAID().getName()+"terminated.");
}
...
super(NaturalGasLoadAgent.this, cfp);
title = t;
maxPrice = p;
manager = m;
enrgy=e;
NaturalGas book = new NaturalGas();
book.setTitle(title);
Sell sellAction = new Sell();
sellAction.setItem(book);
Action act = new Action(NaturalGasLoadAgent.this.getAID(), sellAction);
try {
cfp.setLanguage(codec.getName());
cfp.setOntology(ontology.getName());
NaturalGasLoadAgent.this.getContentManager().fillContent(cfp, act);
} catch (Exception a) {
a.printStackTrace();
}
}
...
NaturalGasInfo NaturalGas=new NaturalGasInfo();
Costs en=new Costs();
if(NaturalGas.getEnergy()<=en.getEnergy())
{
if (rsp == bestOffer) {
...
// NaturalGas successfully purchased
int price = Integer.parseInt(inform.getContent());
myGui.notifyUser("NaturalGas for "+title+" successfully purchased by "+getAID().getLocalName()+". Price =" + price+" from "+NaturalGasSupplierAgent.seller_name);
manager.stop();
}
